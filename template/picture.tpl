...
{combine_script id='tinyscrollbar' load='header' require='jquery' path='themes/PwgCarbon_dft/js/tinyscrollbar.js'}
...
{$ELEMENT_CONTENT}
	...
	<div id="comments">	
		...
		{else}
			<H3>{'There are no comments'|@translate}</h3> 
		{/if}

{if isset($comment_add)}
<div id="adcomm">	
<p>{'Add a comment'|@translate}</p>
	<form method="post" action="{$comment_add.F_ACTION}" class="filter" id="addComment">
		<fieldset>
			...
			{/if}
				<label class="addcom"><textarea name="content" id="contentid" rows="6" cols="100">{$comment_add.CONTENT}</textarea></label>
...
$(document).ready(function(){
	$('#scrollbar1').tinyscrollbar( {sizethumb: 10 , size:100 });	
});
